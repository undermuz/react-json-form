...
type CodePanelProps = {
    ...
    /** Text written to the clipboard; defaults to `code` when `copyable` */
    copyText?: string
    copyable?: boolean
}
...
const CodePanel: FC<CodePanelProps> = ({
    ...
    return (
        ...
        >
            <div className="flex items-center gap-3 border-b border-rpb-border px-4 py-3">
                ...
                {filename ? (
                    <span className="flex-1 text-center font-mono text-xs text-rpb-muted">
                        {filename}
                    </span>
                ) : (
                ...
                {copyable ? (
                    <button
                        type="button"
                        onClick={onCopy}
                        className="shrink-0 rounded-md border border-rpb-border px-2.5 py-1 font-mono text-[11px] text-rpb-muted transition hover:border-rpb-primary/50 hover:text-rpb-text"
                    >
                        {copied ? "Copied" : "Copy"}
                    </button>
                ) : (
