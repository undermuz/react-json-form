...
function isExampleId(
    id: string | undefined,
    views: Record<string, unknown>
): id is ExampleId {
    return Boolean(id && id in views)
}
...
export default function ExamplesPage({
    ...
    const { EXAMPLE_VIEWS, EXAMPLES } = useMemo(
        () => createExampleCatalog(ui, snippets),
        [ui, snippets]
    )
    const { exampleId } = useParams()
    const activeId: ExampleId = isExampleId(exampleId, EXAMPLE_VIEWS)
        ? exampleId
        : "login"
    const ActiveView = EXAMPLE_VIEWS[activeId]

    return (
        <div className="flex min-h-screen w-full flex-col bg-rpb-surface text-rpb-text">
            <AppChrome
                homeHref={homeHref}
            ...
            <main className="flex-1 px-4 py-10 sm:px-6 sm:py-14">
                <div className="mx-auto max-w-6xl">
                    ...
                    <div className="mt-10 grid gap-8 lg:grid-cols-[220px_minmax(0,1fr)] lg:items-start">
                        <nav
                            aria-label="Examples"
                            className="flex gap-2 overflow-x-auto lg:sticky lg:top-36 lg:flex-col lg:overflow-visible"
                        >
                            {EXAMPLES.map((item) => (
                                <NavLink
                                    key={item.id}
