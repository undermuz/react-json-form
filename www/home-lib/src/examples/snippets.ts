...
export type Snippets = {
    ...
    offer: Snippet
    submit: Snippet
    "custom-component": Snippet
    "grid-layout": Snippet
}

export type CreateSnippetsOptions = {
    themePackage: string
    themeImportName: string
    /** e.g. `@undermuz/react-json-form-theme-base/styles.css` — omitted when unused */
    stylesImport?: string
}

function themePreamble({
    themePackage,
    themeImportName,
    stylesImport,
}: CreateSnippetsOptions): string {
    const stylesLine = stylesImport
        ? `import "${stylesImport}"\n`
        : ""
    return `import ${themeImportName} from "${themePackage}"
${stylesLine}`
}

export function createSnippets(options: CreateSnippetsOptions): Snippets {
    const { themeImportName } = options
    const themeImport = themePreamble(options)
...
export default function LoginForm() {
  ...
  return (
    <UiContext.Provider value={${themeImportName}}>
...
${themeImport}import LoginScheme from "./schemes/login"
...
${themeImport}import LayoutScheme from "./schemes/layout"
